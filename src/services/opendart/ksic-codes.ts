// 한국표준산업분류 제10차 (통계청 고시 2017-13호)
// Source: https://github.com/FinanceData/KSIC

export const KSIC_CODES: Record<string, string> = {
  "01": "농업",
  "011": "작물 재배업",
...
export function getIndustryName(code: string): string | null {
  if (KSIC_CODES[code]) return KSIC_CODES[code];
  for (let len = code.length - 1; len >= 2; len--) {
    const parent = code.substring(0, len);
    if (KSIC_CODES[parent]) return KSIC_CODES[parent];
  }
  return null;
}
